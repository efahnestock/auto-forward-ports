...
# Check bash version (need 4.0+ for declare -A)
if (( BASH_VERSINFO[0] < 4 )); then
    echo "Error: auto-forward-ports requires bash 4.0+ (found ${BASH_VERSION})" >&2
...
redraw() {
    ...
    readarray -t sorted_ports < <(printf '%s\n' "${sorted_ports[@]}" | sort -n)
...
while true; do
    mapfile -t lines < <(get_remote_ports)
    declare -a current_port_list=()
    changed=0
